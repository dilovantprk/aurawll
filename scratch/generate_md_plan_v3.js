...
// Compile all updates: User feedback v2 + Simplified translation review
const compiledEdits = {
  ...
  info_heatmap_body: 'Sinir sistemi durumlarının (Ventral, Sempatik, Dorsal) uzun vadeli dağılımını haritalandırır. Bu analiz, temel fizyolojik durumunu ve zaman içindeki dengelenme eğilimlerini belirler.',
  ...
  mc_p_478: 'Kaygı hissettiğinde sinir sistemin alarm durumuna geçer. Uzun nefes vermek ise vücudu yavaşlatıp sakinleştirir.',
  mc_p_sigh: 'Gergin olduğunda nefesin daralır. Üst üste iki kez nefes alıp yavaşça vermek, vücudu en hızlı rahatlatma yoludur.',
  mc_p_grounding: 'Zihnin dağıldığında derin nefes vererek dikkatini bedenine ve ana geri getirebilirsin.',
  mc_p_box: 'Kendini boşlukta veya hayattan kopuk hissettiğinde, düzenli ve eşit nefes almak zihnini yeniden canlandırır.',
  ...
  scan_sigh_0: 'O derin iç çekiş zihnini rahatlattı. Geriye kalan boşluğu ve hafifliği hisset.',
  ...
  market_status_locked: 'Plus Üyelik',
...
let md = `# Aura Türkçe Metinler Düzeltme ve İnceleme Planı (V3 - Son Güncelleme)

Geribildirimleriniz doğrultusunda plan tamamen sadeleştirilmiştir:
1. **Gereksiz "bilimsel mambo-jambo" ve ağır terminolojiler elendi** (baseline, alveolar, vagal fren, parasempatik, interoseptif vb. kelimeler yerine son derece sade ve anlaşılır karşılıklar yazıldı).
2. **Uydurma/mekanik çeviriler atıldı** ("bozuk değilsin" vb. tuhaf ifadeler kaldırıldı, "Oturmasına izin ver" gibi deyimsel olmayan yapılar Türkçeleştirildi).
3. **"Sen" dili tamamen korundu** ve geriye kalan tüm gözden kaçan "Siz" ekleri temizlendi.
4. **Karmaşık/soyut niyetler ve yönergeler somutlaştırıldı** (örn: "Küçük hareket" yerine "Omuzları esnetmek veya su içmek" yazıldı).
5. **Kayıt ve yükleme butonları sadeleştirildi** ("kilitli" yerine "Plus Üyelik" vb.).
6. **"Adım" yazıları kaldırıldı**, sadece "1 / 6" şeklinde sadeleştirildi.

Lütfen son halini inceleyin. Onaylamanız durumunda tüm projeyi bu son sürümle güncelleyeceğim!
...
for (const keyId in groups) {
  ...
  md += `## ${g.title}\n\n`;
  md += `| Anahtar (Key) | İngilizce (EN) | Mevcut Türkçe (TR) | Güncel Öneri (V3) |\n`;
  md += `| :--- | :--- | :--- | :--- |\n`;
...
console.log('Successfully updated implementation_plan.md to v3');
