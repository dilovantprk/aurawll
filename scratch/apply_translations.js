...
const filePath = '/Users/dilovan/Documents/antigrvty/translations.js';
let content = fs.readFileSync(filePath, 'utf8');

// Compiled edits (Natural Turkish)
const edits = {
  // Navigation & Tabs
...
// Locate where 'tr: {' starts in translations.js
const trStartPattern = 'tr: {';
const trStartIndex = content.indexOf(trStartPattern);

if (trStartIndex === -1) {
  console.error('Could not find the start of the tr block.');
  process.exit(1);
}

const beforeTr = content.substring(0, trStartIndex);
let trBlock = content.substring(trStartIndex);

let replacedCount = 0;

for (const [key, newValue] of Object.entries(edits)) {
  // We want to match "key": "value", or "key": 'value', or "key": `value`, inside the tr block
  // Handle double quotes and escape characters in the replacement
  const escapedValue = newValue.replace(/"/g, '\\"');
  
  // Match the exact pattern: "key": "anything"
  const regex = new RegExp(`"${key}"\\s*:\\s*"[^"]*"`, 'g');
  
  if (regex.test(trBlock)) {
    trBlock = trBlock.replace(regex, `"${key}": "${escapedValue}"`);
    replacedCount++;
  } else {
    // Try with single quotes if any
    const regexSingle = new RegExp(`"${key}"\\s*:\\s*'[^']*'`, 'g');
    if (regexSingle.test(trBlock)) {
      trBlock = trBlock.replace(regexSingle, `"${key}": "${escapedValue}"`);
      replacedCount++;
    } else {
      console.warn(`Key not found in tr section: ${key}`);
    }
  }
}

fs.writeFileSync(filePath, beforeTr + trBlock, 'utf8');
console.log(`Successfully applied ${replacedCount} translation edits inside tr section.`);
