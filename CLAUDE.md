...
<!-- END KIT CORE -->
## Project
- Run health checks before reporting done: `bash scripts/health-check.sh` (fails on broken imports, orphan files, root JS)
- Write JS code ONLY in `js/` subdirectory (root JS is forbidden).
- Respect 3-tier JS layers: `js/core/` (pure logic/state), `js/components/` (UI), `js/services/` (external/audio).
- `app.js` is entry/orchestrator only (no business logic).
- Relative imports from components go up one level (e.g. `../core/state.js`). Never import `../../`.
- Write CSS ONLY in `css/components/*.css`. Import new CSS files inside `style.css` (do not write rules in style.css).
- `css/base.css` is tokens, reset, forms, buttons only. `css/layout.css` is layout grid, nav, breakpoints only.
- Never write to, import from, or refer to `_archive/` files (deprecated files).
- Keep translations in `translations.js`.
